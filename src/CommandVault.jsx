...
const CommandVault = () => {
  const [commands, setCommands] = useState([
    ...
    { id: 2, name: 'Check Sysmon Status', command: '$logman query "NT6 Operational" -ets', description: 'Verify if Sysmon event logging is active', tags: ['security', 'forensics', 'diagnostics'], lastUsed: new Date(Date.now() - 1000 * 60 * 30), usageCount: 18 },
    { id: 3, name: 'Find Recent File Changes', command: 'Get-ChildItem -Path C:\\ -Recurse -Force | Where-Object { $_.LastWriteTime -gt (Get-Date).AddDays(-7) }', description: 'Scan for files modified in last 7 days', tags: ['incident-response', 'forensics'], lastUsed: new Date(Date.now() - 1000 * 60 * 120), usageCount: 12 },
  ...
  const copyCommand = useCallback((cmd) => {
    ...
    setCommands(prevCommands => prevCommands.map(c => c.id === cmd.id ? { ...c, lastUsed: new Date(), usageCount: c.usageCount + 1 } : c));
  }, []);
  ...
  const handleEditCommand = () => {
    if (!formData.name || !formData.command) return;
    setCommands(prevCommands => prevCommands.map(c => c.id === editingCommand.id ? { ...c, name: formData.name, command: formData.command, description: formData.description, tags: formData.tags.split(',').map(t => t.trim()).filter(t => t) } : c));
    setFormData({ name: '', command: '', description: '', tags: '' });
  ...
  const handleDeleteCommand = () => {
    setCommands(prevCommands => prevCommands.filter(c => c.id !== editingCommand.id));
